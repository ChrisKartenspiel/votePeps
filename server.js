...
mongoose.connection.on('error', function(){
  console.info('Error:COud not connect to MongoDB.')
});
        ...
        parser.parseString(xml, function(err,parsedXml){
          ...
          try {
            ...
            character.save(function(err){
              if(err) return next(err);
              res.send({ message: characterName + ' has been added succesfully!'});
            });
          } catch (e){
            res.send(404).send({ message: characterName + ' is not a registered citizen of New Eden.'});
          }
        });
      });
    }
...
app.get('/api/characters', function(req,res,next){
  var choises = ['Female', 'Male'];
  var randomGender = _.sample(choises);
  ...
  Character
    ...
    .exec(function(err,characters){
      ...
      var oppositeGender = _.first(_.without(choises,randomGender))
...
app.get('/api/characters/search',function(req,res,next){
  var charaterName = new RegExp(req.query.name, 'i');
...
app.post('/api/report', function(req,res,next){
  ...
  Character.findOne({ characterId:characterId}, function(err,character){
    ...
    if(character.reports>4){
      character.remove();
      return res.send({ message: character.name + ' had more than 4 reports, thus has been deleted.'})
    }
